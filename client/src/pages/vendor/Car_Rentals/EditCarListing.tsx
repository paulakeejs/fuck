...
const mapBackendEnum = (value: string, options: string[]) => {
  if (!value) return "";
  // Try to match ignoring case and underscores
  const normalized = value.replace(/_/g, " ").toLowerCase();
  return (
    options.find(
      (opt) => opt.replace(/-/g, " ").toLowerCase() === normalized
    ) || value
  );
};
...
function EditCarListing() {
  ...
  const getCar = async () => {
    try {
      const response = await carApi.get("/edit/" + id);
      if (response.data.success) {
        ...
        setCarType(mapBackendEnum(c.carType, CAR_TYPES) || "");
        setColor(c.color || "");
        setInteriorColor(c.interiorColor || "");
        setSeats(c.seats ? String(c.seats) : "");
        setDoors(c.doors ? String(c.doors) : "");
        setTransmission(mapBackendEnum(c.transmission, TRANSMISSIONS) || "");
        setDriveType(mapBackendEnum(c.driveType, DRIVE_TYPES) || "");
        setFuel(mapBackendEnum(c.fuel, FUEL_TYPES) || "");
        setCondition(mapBackendEnum(c.condition, CONDITION_LEVELS) || "");
        setPrice(c.price ? String(c.price) : "");
        setWeeklyDiscount(
          c.weeklyDiscount !== undefined && c.weeklyDiscount !== null
            ? String(c.weeklyDiscount)
            : ""
        );
        setMonthlyDiscount(
          c.monthlyDiscount !== undefined && c.monthlyDiscount !== null
            ? String(c.monthlyDiscount)
            : ""
        );
        setDeposit(
          c.deposit !== undefined && c.deposit !== null ? String(c.deposit) : ""
        );
        setVin(c.vin || "");
        setLicensePlate(c.licensePlate || "");
        setDeliveryOption(
          mapBackendEnum(c.deliveryOption, [
            "none",
            "local",
            "airport",
            "custom",
        ...
        setInsuranceInfo(c.insuranceInfo || "");
        setRentalTerms(c.rentalTerms || "");

        setSelectedFeatures(Array.isArray(c.features) ? c.features : []);
        setIsFeatured(!!c.isFeatured);
        setIsAvailable(
          c.isAvailable === undefined || c.isAvailable === null
            ? true
            : !!c.isAvailable
        );
        setMileage(
          c.mileage !== undefined && c.mileage !== null ? String(c.mileage) : ""
        );
        setEngineSize(c.engineSize || "");
        setHorsepower(
          c.horsepower !== undefined && c.horsepower !== null
            ? String(c.horsepower)
            : ""
        );
        setAcceleration(
          c.acceleration !== undefined && c.acceleration !== null
            ? String(c.acceleration)
            : ""
        );
        setTopSpeed(
          c.topSpeed !== undefined && c.topSpeed !== null
            ? String(c.topSpeed)
            : ""
        );
        setCarImages(Array.isArray(c.images) ? c.images : []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await carApi.put(`/edit/${id}`, payload);
      if (response.data.success) {
    ...
    try {
      const res = await fetch(
        ...
        )}&limit=10&sort=-population`,
        {
          headers: {
            "X-RapidAPI-Key":
              "72a3073863msh8e55c9def9e10c8p1912f1jsnc072631e094b",
  ...
  const handleRemoveImage = (url: string) => {
    setCarImages((prev) => prev.filter((img) => img !== url));
  };
                  ...
                  {carImages.map((url, idx) => (
                    <div key={idx} className="relative group">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 bg-white bg-opacity-80 rounded-full p-1 text-red-600 hover:bg-red-100 transition group-hover:scale-110"
                        title="Remove image"
                      >
                        &times;
                      </button>
