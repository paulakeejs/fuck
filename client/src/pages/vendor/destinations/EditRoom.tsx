...
export default function EditRoom() {
  ...
  useEffect(() => {
    const fetchRoomData = async () => {
      if (!id) {
        setErrors({ submit: "Invalid room ID" });
        setLoading(false);
        return;
      }
      try {
        const response = await destinationApi.get(`/edit/${id}`);
        if (!response.data.success) {
          throw new Error(response.data.error || "Failed to fetch room data");
        }
        ...
        setFormData({
          ...
          minimumStay: room.minimumStay?.toString() || "",
          ...
          extraFees:
            room.extraFees?.map((fee: ExtraFee) => ({
              name: fee.name,
              amount: fee.amount.toString(),
            })) || [],
        });
        setRoomImages(room.images?.map((img: any) => img.url) || []);
        setImageDescriptions(
          room.images?.reduce(
            (acc: { [key: string]: string }, img: any, idx: number) => ({
              ...acc,
              [idx]: img.description || "",
            }),
            {}
        ...
        setErrors({
          submit:
            err instanceof Error
              ? err.message
              : "Failed to fetch room data. Please try again.",
        });
      } finally {
        setLoading(false);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await destinationApi.put(`/update/${id}`, roomData);

      if (!response.data.success) {
        throw new Error(response.data.error || "Failed to update room");
      }
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-navy/95 py-8 px-4 flex items-center justify-center">
        <motion.div
          animate={{ rotate: 360 }}
          transition={{ repeat: Infinity, duration: 1 }}
          className="h-12 w-12 border-4 border-t-gold border-r-gold border-b-cream border-l-cream rounded-full"
        />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-navy/95 py-8 px-4 w-[900px]">
      <title>Edit Room | Luxvana International</title>
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <p className="text-gold/80 font-montserrat">
            Update the luxury experience for your guests
          </p>
        </motion.div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`${inputClass} pl ZigZag} pl-10`}
                    placeholder="e.g., 10"
                    min="0"
                    aria-required="true"
                  />
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className={`${buttonClass} bg-black text-sm px-4 py-2`}
                >
