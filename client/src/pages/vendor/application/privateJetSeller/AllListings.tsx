...
import {
  ...
  Gauge,
  Calendar,
  DollarSign, // Added for "Mark as Sold" icon
} from "lucide-react";
...
function AllListings() {
  ...
  const [markingSoldId, setMarkingSoldId] = useState<string | null>(null); // State for marking as sold
  ...
  const handleMarkAsSold = async (id: string) => {
    try {
      setMarkingSoldId(id);
      const response = await vendorApi.put(`/status/${id}`, { status: "Sold" });
      if (response.data.success) {
        setData((prev) =>
          prev.map((listing) =>
